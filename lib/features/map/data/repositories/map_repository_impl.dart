...
class MapRepositoryImpl implements MapRepository {
  ...
  @override
  Future<List<MapLocation>> getNearbyLocations({String? type, String? search, LatLng? userLocation, LatLngBounds? bounds}) async {
    ...
    if (query.isEmpty) {
      if (type == 'workshop') query = 'taller mecanico';
      else if (type == 'gasstation') query = 'gasolinera';
    }
    ...
    if (query.isNotEmpty || type == 'all' || type == 'workshop' || type == 'gasstation') {
      geoResults = await _geocodingService.searchAddress(query, bounds: bounds, userLocation: userLocation, type: type);
    }
